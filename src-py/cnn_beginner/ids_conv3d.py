...
# Reshape data for Conv3D: (samples, depth, height, width, channels)
# Here we use depth=1, height=1, width=number_of_features, channels=1
X_train = X_train.reshape((X_train.shape[0], 1, 1, X_train.shape[1], 1))
X_test = X_test.reshape((X_test.shape[0], 1, 1, X_test.shape[1], 1))

# Define the model
# e.g., (1, 1, 41, 1) for 41 features treated as a 1x1x41 "volume" with 1 channel
input_shape = (1, 1, X_train.shape[3], 1)
model = Sequential()
model.add(Input(shape=input_shape))
model.add(Conv3D(filters=32, kernel_size=(1, 1, 3), activation='relu'))
